...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="50" height="50" alt="handle" src="'+o.imagePath+'handle.png" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			$('div:eq(2)', obj).height(imgHeight).width(imgWidth*o.introPosition).css({'position':'absolute','overflow':'hidden','right':'0px','left':'0px','z-index':'10'}); // Set CSS properties of the before image div
			$('div:eq(3)', obj).height(imgHeight).width(imgWidth).css({'position':'absolute','overflow':'hidden','left':'0px'});	// Set CSS properties of the after image div
			$('#drag'+randID).width(2).height(imgHeight).css({'background':o.dividerColor,'position':'absolute','left':'25px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			$('#handle'+randID).css({'z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'-25px'})
			...
			//I just add a container for the dragging zone
			$('#dragwrapper'+randID).draggable( {containment : [-25,0,140,0], drag:drag,stop:drag });
			...
			function drag()
			{
            	...
            	if (xPos<70)//if the selector is on the left (<80 px)
            	{
            		$('#logozero').fadeIn(600);//we display the logo
            	}
            	else{
            		$('#logozero').fadeOut(600);//if not we don't display it
            	}
			}
			...
			{		
				...
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				//// I just modified the parameter of the function that makes the selector follewing the handler. If raises an click and TOUCH event(l.169)////
				//// in order that the animations looks the same from desktop to phone/tablet. Benoit. ///////////////////////////////////////////////////////
				//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				

				// When clicking in the container, move the bar and imageholder divs
				$(obj).on("click touch",function(e){
